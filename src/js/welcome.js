...
const createRepo = (token, name) => {
  ...
  let data = {
    ...
    auto_init: true,
    description:
      'Collection of LeetCode questions to ace the coding interview! - Created using [LeetHub 5.0](https://github.com/14-Tanmay/LeetHubV5)',
  };
...
$('#sync_counts').on('click', async () => {
  ...
  // Fetch stats directly from LeetCode
  const fetchLeetCodeStats = async () => {
    try {
      $('#success').text('Syncing counts from LeetCode...').show();
      const userRes = await fetch('https://leetcode.com/graphql/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ query: 'query globalData { userStatus { username } }' }),
      });
      const userData = await userRes.json();
      const username = userData?.data?.userStatus?.username;

      if (!username) throw new Error('No username found. Are you logged in to LeetCode?');

      const statsQuery = `query getUserProfile($username: String!) { matchedUser(username: $username) { submitStats { acSubmissionNum { difficulty count } } } }`;
      ...
      if (acSubmissions) {
        ...
        const total = acSubmissions.find(s => s.difficulty === 'All')?.count || 0;
        ...
        // Update local storage stats cache
        stats.solved = total;
        stats.easy = easy;
        stats.medium = medium;
        stats.hard = hard;
        chrome.storage.local.set({ stats }, () => {
          $('#success').text('Successfully synced counts from LeetCode!').show();
          setTimeout(() => {
            $('#success').hide();
          }, 3000);
        });
      ...
      $('#success').hide();
      $('#error')
        .text('Failed to fetch LeetCode stats. Please ensure you are logged into LeetCode.')
        .show();
    }
  };
